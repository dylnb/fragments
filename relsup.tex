...
\begin{align*} % Absolute Definite Deriv
  ...
  &
  \btower{%
    \l gh \col \one_v^\ms{G} \land \one_u^\ms{G'} \dt
    \uset{%
...
\begin{align*} % Haddock Deriv
  ...
  \p{%
    ...
    \p{%
      ...
      %
      \btower{%
        \l gh \dt \uset{\hole\,g^{v\mapsto y}\,h \giv \cn{sq}\,y},
        y
      } } }
    } }
  }^{\dnar\,\reset} \\
  ...
  &
  \btower{%
    \l gh \col \one_u^\ms{G} \land \one_v^\ms{G} \dt
    \uset{%
      \hole\,g^{\substack{u\mapsto x\\ v\mapsto y}}\,h
    \giv
      \cn{sq}\,y,\ \cn{circ}\,x,\ \cn{in}\,y\,x
...
\begin{itemize} % Haddock Notes
  ...
  \item
    For any input $g$, the set of outputs that $\one_u$ and $\one_v$ will
    target is
    $\set{%
    ...
    All of the outputs in this set will map $u$ to some circle and $v$ to some
    square containing the circle they map to $u$.
  \item
...
\begin{align*} % DP Sup Reset Deriv
  ...
  &\leadsto
  \p{%
    \btower{\l g \dt \M_v^\cn{ag}\uset{\hole\,g^{v\mapsto y} \giv \cn{sq}\,y}, y}
  }^\reset
  %
  \leadsto
  \btower{%
