...
-- (quantifier-internal) anaphoric "same" with de Groote continuations
...
-- Check if an Ent is plural
is_pl :: Ent -> Maybe [Ent]
is_pl (Plur i) = Just i
is_pl (Atom i) = Nothing

-- return the nth element on a stack, if it's plural; otherwise return the
--   join of the first two items on the stack
get_pl :: Stack -> Int -> [Ent]
get_pl xs n | isNothing (is_pl (xs!!n))     = [xs!!0, xs!!1]
            | otherwise                     = fromJust (is_pl (xs!!n))
...
-- "every" is a normal GQ, except that it writes the sum of its restrictor
--   onto the nth position in the stack (for quantifier-internal retrieval)
every :: Int -> (Ent -> Prop) -> GQ
every n res scope i k = 
  and [not (res x i (\i' -> not (scope x (ins n (top res) (x:i')) trivial)))
        | x <- domain] && (k i)

-- for uniformity, "some" and "no" place the variables they quantify over 
--   on the nth position in the stack (but not the sum of their restrictors)
some :: Int -> (Ent -> Prop) -> GQ
some n res scope i k =
  or [res x i (\i' -> scope x (ins n x i') k) | x <- domain]

no :: Int -> (Ent -> Prop) -> GQ
no n res scope i k =
  and [not (res x i (\i' -> scope x (ins n x i') trivial)) | x <- domain] && (k i)
...
--   here
the :: Int -> (Ent -> Prop) -> GQ
the n res scope i k = scope (top res) (ins n (top res) i) k

-- Adjectives
-- ---------------------------------------- 

-- index on "same"/"different" must match index on distributor
same, different :: Int -> (Adj -> GQ) -> (Ent -> Ent -> Prop) -> Ent -> Prop
same n dp verb x i k =
  ...
  and [eval (dp (\g -> \y -> conj (verb y u) (g y)) (\z -> verb z v))
        | u <- atoms, v <- (delete u domain),
          elem u (get_pl i n), fn v (get_pl i n)] && (k i)

different n dp verb x i k =
  let fn s@(Atom x) = elem s
      fn   (Plur x) = (\w -> all (\r -> elem r w) x) in
